...
const WORLDNEWS_API_KEY = import.meta.env.VITE_WORLDNEWS_API_KEY;
...
function App() {
  ...
  useEffect(() => {
    const fetchAllNews = async () => {
      ...
      const worldNewsApiUrl = `https://api.worldnewsapi.com/search-news?text=Artificial Intelligence&language=en&api-key=${WORLDNEWS_API_KEY}`;
      ...
      const results = await Promise.allSettled([
        fetch(worldNewsApiUrl).then((res) => res.json()),
      ...
      if (results[0].status === "fulfilled" && results[0].value.news) {
        const normalized = results[0].value.news.map((article) => ({
          id: article.id, // Menggunakan ID unik dari World News API
          title: article.title,
          url: article.url,
          imageUrl: article.image,
          source: article.source_country,
          publishedAt: article.publish_date,
          apiSource: "World News",
        }));
        combinedArticles.push(...normalized);
      } else {
        console.error(
          "Failed to fetch from World News API:",
          results[0].reason || results[0].value
        );
      }
      ...
      if (
        ...
        results[2].value.response?.results
      ) {
  ...
  useEffect(() => {
    const savedBookmarks = JSON.parse(
      sessionStorage.getItem("newsBookmarks") || "[]"
    );
  ...
  const removeBookmark = (articleId) => {
    setBookmarks((prev) =>
      prev.filter((bookmark) => bookmark.id !== articleId)
    );
  };
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="py-2 sm:py-3 lg:py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div
                  className={`hidden sm:flex items-center rounded-xl p-1 transition-colors duration-300 ${
                ...
                {/* Tab Navigation */}
                <div
                  className={`hidden sm:flex rounded-xl p-1 transition-colors duration-300 ${
                    darkMode ? "bg-gray-700" : "bg-gray-100"
                  }`}
                >
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${
                      !showBookmarks
                        ? darkMode
                          ? "bg-gray-600 shadow-md text-white"
        ...
        {mobileMenuOpen && (
          <div
            className={`sm:hidden border-t transition-colors duration-300 ${
              darkMode
      ...
      <main className="max-w-8xl mx-auto px-2 sm:px-4 lg:px-6 xl:px-8 py-3 sm:py-4 lg:py-6 xl:py-8">
        ...
        {loading && (
          <div className="flex items-center justify-center py-12 lg:py-20">
            <div
              className={`flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 transition-colors duration-300 ${
                darkMode ? "text-gray-300" : "text-gray-600"
              }`}
            >
              <div
                className={`w-8 h-8 border-3 rounded-full animate-spin transition-colors duration-300 ${
                  darkMode
        ...
        {error && (
          <div
            className={`border rounded-xl p-4 sm:p-6 mb-6 transition-colors duration-300 ${
              darkMode
        ...
        {!loading && !error && (
          <>
            {/* Stats Bar */}
            <div
              className={`rounded-xl shadow-sm border p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 transition-all duration-300 ${
                darkMode
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                  ...
                  >
                    <span className="flex items-center space-x-2">
                      <span>Showing</span>
                      <span
                        className={`px-2 py-1 rounded-lg font-semibold text-sm transition-colors duration-300 ${
                          darkMode
                  ...
                  {searchQuery && (
                    <div
                      className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors duration-300 ${
                        darkMode
            ...
            {currentArticles.length > 0 ? (
                ...
                {totalPages > 1 && (
                  <div className="flex flex-col items-center space-y-6">
                    ...
                    <div className="flex sm:hidden items-center justify-between w-full max-w-sm">
                      ...
                      <div
                        className={`flex items-center space-x-3 px-4 py-2 rounded-xl transition-colors duration-300 ${
                    ...
                    {/* Pagination Info */}
                    <div
                      className={`text-center space-y-1 transition-colors duration-300 ${
                        darkMode ? "text-gray-400" : "text-gray-500"
                      }`}
                    >
              ...
              /* Empty State */
              <div
                className={`rounded-xl border p-8 sm:p-12 lg:p-16 text-center transition-colors duration-300 ${
                  darkMode
                    ? "bg-gray-800 border-gray-700"
                    : "bg-white border-gray-200"
                }`}
              >
                <div className="max-w-lg mx-auto">
                  <div
                    className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 transition-colors duration-300 ${
                      darkMode ? "bg-gray-700" : "bg-gray-100"
                    }`}
                  >
                    {showBookmarks ? (
      ...
      {/* Footer */}
      <footer
        className={`border-t mt-12 sm:mt-16 lg:mt-20 transition-colors duration-300 ${
          darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
        }`}
      >
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 lg:py-12">
          <div className="text-center space-y-4">
            ...
            <p
              className={`text-sm sm:text-base max-w-2xl mx-auto transition-colors duration-300 ${
                darkMode ? "text-gray-400" : "text-gray-500"
              }`}
            >
              Stay informed with the latest AI news from trusted sources
